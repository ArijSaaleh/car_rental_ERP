...
export class TenantInterceptor implements NestInterceptor {
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (user) {
      ...
      else if (user.role === UserRole.PROPRIETAIRE && request.method === 'POST' && request.path === '/agencies') {
      ...
      else if (user.role === UserRole.PROPRIETAIRE && !user.agencyId && request.path.startsWith('/agencies')) {
