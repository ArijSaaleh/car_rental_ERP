...
async def get_available_vehicles(
    ...
    Role-based access:
    - Manager/Employees: Auto-use their assigned agency
    - Proprietaire/Super Admin: Must provide agency_id parameter OR use their own if they have one
    """
    # Determine target agency
    target_agency_id = agency_id
    ...
    if current_user.role in [UserRole.MANAGER, UserRole.AGENT_COMPTOIR, UserRole.AGENT_PARC]:  # type: ignore
        ...
        # For PROPRIETAIRE and SUPER_ADMIN
        if not target_agency_id:
            # Try to use their agency_id if they have one
            if current_user.agency_id:  # type: ignore
                target_agency_id = current_user.agency_id  # type: ignore
            else:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
...
async def create_booking(
    ...
    # Cast to UUID for type safety
    agency_uuid: UUID = target_agency_id  # type: ignore
    
    # Verify agency access
    verify_agency_access(current_user, agency_uuid, db)
    ...
    pricing = BookingService.calculate_rental_price(
        ...
        end_date=booking_data.end_date,
        agency_id=agency_uuid,
        daily_rate=float(booking_data.daily_rate) if booking_data.daily_rate else None
    )
    
    # Generate booking number
    booking_number = BookingService.generate_booking_number(db, agency_uuid)
    ...
    try:
        new_booking = Booking(
            ...
            created_by_user_id=current_user.id,  # type: ignore
            start_date=booking_data.start_date,
            end_date=booking_data.end_date,
            daily_rate=float(booking_data.daily_rate) if booking_data.daily_rate else pricing["daily_rate"],
            duration_days=pricing["duration_days"],
            subtotal=pricing["subtotal"],
            tax_amount=pricing["tax_amount"],
            timbre_fiscal=pricing["timbre_fiscal"],
            total_amount=pricing["total_amount"],
            deposit_amount=float(booking_data.deposit_amount) if booking_data.deposit_amount else 0.0,
            mileage_limit=booking_data.mileage_limit,
            extra_mileage_rate=float(booking_data.extra_mileage_rate) if booking_data.extra_mileage_rate else None,
...
async def list_bookings(
    ...
    try:
        ...
        if current_user.role in [UserRole.MANAGER, UserRole.AGENT_COMPTOIR, UserRole.AGENT_PARC]:  # type: ignore
            ...
            # For PROPRIETAIRE/SUPER_ADMIN: use provided agency_id or fallback to user's agency
            if not target_agency_id:
                if current_user.role == UserRole.PROPRIETAIRE and current_user.agency_id:  # type: ignore
                    target_agency_id = current_user.agency_id  # type: ignore
                else:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
...
async def confirm_booking(
    ...
    Role-based access:
    - Only MANAGER and above can confirm bookings
    - Automatically verifies access to booking's agency
    """
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    ...
    # Verify vehicle availability hasn't changed
    is_available = BookingService.check_vehicle_availability(
        db=db,
        vehicle_id=booking.vehicle_id,
        start_date=booking.start_date,
    ...
    if not is_available:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Vehicle is no longer available for this period"
        )
...
async def start_rental(
    booking_id: int,
    initial_mileage: int,
    initial_fuel_level: str,
    current_user: User = Depends(check_permission(UserRole.AGENT_COMPTOIR)),
    db: Session = Depends(get_db)
):
    """
    Start a rental (CONFIRMED -> IN_PROGRESS)
    
    Role-based access:
    - AGENT_COMPTOIR and above can start rentals
    - Automatically verifies access to booking's agency
    """
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    ...
    if booking.status != BookingStatus.CONFIRMED:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Cannot start rental with status: {booking.status}"
        )
    
    # Update booking
    booking.status = BookingStatus.IN_PROGRESS
    booking.pickup_datetime = datetime.now()
    booking.initial_mileage = initial_mileage
    booking.initial_fuel_level = initial_fuel_level
...
async def complete_rental(
    booking_id: int,
    final_mileage: int,
    final_fuel_level: str,
    current_user: User = Depends(check_permission(UserRole.AGENT_COMPTOIR)),
    db: Session = Depends(get_db)
):
    """
    Complete a rental (IN_PROGRESS -> COMPLETED)
    
    Role-based access:
    - AGENT_COMPTOIR and above can complete rentals
    - Automatically verifies access to booking's agency
    """
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    ...
    if booking.status != BookingStatus.IN_PROGRESS:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Cannot complete rental with status: {booking.status}"
        )
    
    # Update booking
    booking.status = BookingStatus.COMPLETED
    booking.return_datetime = datetime.now()
    booking.final_mileage = final_mileage
    booking.final_fuel_level = final_fuel_level
    
    # Update vehicle status back to available
    vehicle = db.query(Vehicle).filter(Vehicle.id == booking.vehicle_id).first()
    if vehicle:
        vehicle.status = VehicleStatus.DISPONIBLE
        vehicle.kilometrage = final_mileage
...
@router.post("/{booking_id}/payment", status_code=status.HTTP_200_OK)
async def record_payment(
    booking_id: int,
    amount: float,
    payment_method: str = Query(..., description="cash, card, transfer, check"),
    payment_type: str = Query("rental", description="rental, deposit, extra, refund"),
    ...
    Role-based access:
    - AGENT_COMPTOIR and above can record payments
    - Automatically verifies access to booking's agency
    """
    booking = db.query(Booking).filter(Booking.id == booking_id).first()
    ...
    # This is a simplified version - in production you'd track payments in a separate table
    if payment_type == "deposit":
        booking.deposit_amount = float(amount)
...
@router.get("/{booking_id}/payment-summary")
async def get_payment_summary(
    ...
    # This is simplified - in production you'd aggregate from a payments table
    paid_amount = 0.0
    if booking.payment_status == "paid":
        paid_amount = total_amount
    elif booking.payment_status == "partial":
        paid_amount = total_amount * 0.5  # Placeholder
    ...
    return {
        ...
        "payment_status": booking.payment_status,
        "timbre_fiscal": float(booking.timbre_fiscal),
