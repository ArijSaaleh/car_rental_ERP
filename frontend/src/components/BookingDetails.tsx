...
export function BookingDetails({ booking, open, onClose, onUpdate }: BookingDetailsProps) {
  ...
  const loadPaymentSummary = async () => {
    try {
      // TODO: Implement payment summary loading
      // const summary = await paymentService.getByBooking(booking.id);
  ...
  const handleStartRental = async () => {
    ...
    setError('');
    try {
      await bookingService.start(booking.id, {
        initialMileage: parseInt(actionData.mileage),
        initialFuelLevel: actionData.fuelLevel,
      });
      onUpdate?.();
  ...
  const handleCompleteRental = async () => {
    ...
    try {
      await bookingService.complete(booking.id, {
        finalMileage: parseInt(actionData.mileage),
        finalFuelLevel: actionData.fuelLevel,
      });
      onUpdate?.();
      setStatusDialog(false);
      onClose();
  ...
  const handleRecordPayment = async () => {
    ...
    setError('');
    try {
      // TODO: Implement payment recording using payment service
    ...
    <>
      <Dialog open={open} onOpenChange={onClose}>
        <DialogContent>
          <DialogHeader>
            <div className="flex items-start justify-between">
              <div>
                ...
                <DialogDescription className="text-sm text-slate-500 mt-1">#{booking.bookingNumber}</DialogDescription>
              </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <div className="flex justify-between">
                  ...
                  <p className="text-sm">{parseFloat(booking.subtotal || '0').toFixed(3)} DT</p>
                </div>
                <div className="flex justify-between">
                  <p className="text-sm text-slate-500">TVA</p>
                  <p className="text-sm">{parseFloat(booking.taxAmount || '0').toFixed(3)} DT</p>
                </div>
                <div className="flex justify-between">
                  <p className="text-sm text-slate-500">Timbre fiscal</p>
                  <p className="text-sm">{parseFloat(booking.timbreFiscal || '0').toFixed(3)} DT</p>
                </div>
                <hr className="my-2" />
                ...
                {booking.depositAmount && parseFloat(booking.depositAmount) > 0 && (
                  <div className="flex justify-between">
                    ...
                    <p className="text-sm">{parseFloat(booking.depositAmount).toFixed(3)} DT</p>
                  </div>
                )}
              </CardContent>
            </Card>
          ...
          {(booking.initialMileage || booking.finalMileage) && (
            <Card className="mt-4">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
              ...
              <CardContent className="grid grid-cols-2 gap-4">
                {booking.initialMileage && (
                  <div>
                    ...
                    <p className="font-medium">{booking.initialMileage} km</p>
                  </div>
                )}
                {booking.finalMileage && (
                  <div>
                    <p className="text-sm text-slate-500">Kilométrage final</p>
                    <p className="font-medium">{booking.finalMileage} km</p>
                  </div>
                )}
                {booking.initialFuelLevel && (
                  <div>
                    <p className="text-sm text-slate-500">Niveau carburant initial</p>
                    <p className="font-medium">{booking.initialFuelLevel}</p>
                  </div>
                )}
                {booking.finalFuelLevel && (
                  <div>
                    <p className="text-sm text-slate-500">Niveau carburant final</p>
                    <p className="font-medium">{booking.finalFuelLevel}</p>
                  </div>
                )}
                {booking.fuelPolicy && (
                  <div>
                    <p className="text-sm text-slate-500">Politique carburant</p>
                    <p className="font-medium">{booking.fuelPolicy}</p>
                  </div>
                )}
              </CardContent>
            </Card>
      ...
      <Dialog open={paymentDialog} onOpenChange={setPaymentDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Saisissez les détails du paiement</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={statusDialog} onOpenChange={setStatusDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {booking.status === 'CONFIRMED' 
                ? 'Enregistrer le kilométrage et le niveau de carburant au départ'
                : 'Enregistrer le kilométrage et le niveau de carburant au retour'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div>
